...
export default function FormComponent() {
    ...
    useEffect(() => {
        ...
        const socket = new WebSocket(`ws://localhost:8000/ws/api/check/proccess/${taskId}/`);
    ...
    const createTrip = async () => {
        ...
        setDisable(true);
        const response = await axios.post("http://127.0.0.1:8000/api/trip/create/", {
            serializer: {
    ...
    const debouncedSearch = useCallback(
        debounce(async (query) => {
            const response = await axios.get(`http://127.0.0.1:8000/api/trip/search/?q=${query}&task_id=${taskId === null ? "null" : taskId}`);
